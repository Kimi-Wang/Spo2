...
#include "msg_receive_hander.h"
#include "device_id_operate.h"
...
notify_free_handler_t notify_free_handler = NULL;

uint32_t sample_info_buffer[10][4]={0};//存储flash中数据端的信息（第N端数据+采样率+数据段内的采样数+数据点开始地址）

void notify_msg_scheduler_event_handler(void *p_event_data, uint16_t event_size)
{
    ret_code_t ret;
...
void notify_msg_evt_schedule(int force) //button 
{
    uint32_t ret = 0;
...
ret_code_t notify_msg_send(uint8_t *data, int len)
{
	...
	SEGGER_RTT_printf(0,"--->here notify_msg_send \r\n");
	
    return err_code;
}

static uint32_t tmp_time[3];       //当前时间的缓存
uint8_t rd_size = 0;               //正在采集的buffer大小,但没来的及存到flash中。
uint32_t ch_rd_data_tm = 5;                                 //改变的采样率
uint32_t cur_rd_data_tm = 5;                                //当前实际采样率
static uint32_t dt_num = 0;                                 //数据段中存储的数据个数

//根据收到的指令执行相关操作
void msg_scheduler_event_handler(void *p_event_data, uint16_t event_size)
{
...
//根据传过来的当前时间和采样的总耗时推算出开始时间
void get_start_time(uint8_t *cur_time)
{
...
uint32_t get_samples_num_sec(uint32_t *total_second)
{
...
void data_transmit_format(uint8_t * msg_dat,uint32_t sample_start_point,uint32_t length)//将需求的数据按一定的格式发送出去
{
	uint8_t * start_data ;
	char data_segment=0;
